...
class Essay extends CI_Controller
{
    ...
    public function essay_scoring_view($kd_soal, $npm)
    {
        ...
        $this->data_mahasiswa = $this->essay_model->show_data('npm, nama_mahasiswa, kelas', 'mahasiswa', ['npm' => $npm]);
        ...
        $this->load->view('template/header');
        $this->load->view('essay_scoring_view', ['kd_soal' => $kd_soal, 'mahasiswa' => $this->data_mahasiswa, 'jawaban' => $this->jawaban_essay, 'soal' => $this->soal_matakuliah, 'hasil' => $this->hasil_algoritma]);
        $this->load->view('template/footer');
    }

    public function mahasiswa_view($kd_soal)
    {
        $this->session->set_userdata('kd_soal', $kd_soal);
        $this->data_mahasiswa = $this->essay_model->show_data(column: 'npm, nama_mahasiswa, kelas, program_studi', table: 'mahasiswa');
        $this->load->view('template/header');
        $this->load->view('mahasiswa_view', ['kd_soal' => $kd_soal, 'mahasiswa' => $this->data_mahasiswa]);
        $this->load->view('template/footer');
    }
}
