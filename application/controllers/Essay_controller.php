...
class Essay_Controller extends Essay {
    ...
    public function add_data_soal() {
        if($this->input->method() === 'post') {
            ...
            $patternoal_saved = $this->essay_model->add_data(table: 'soal_esai', data: $this->soal_matakuliah);
    ...
    public function add_jawaban_mhs() {
        if($this->input->method() === 'post') {
            ...
            $final_score = $this->essay_scoring(jawaban: $this->jawaban_essay['jawaban'], kunci_jawaban: $this->input->post('input_kunci_jawaban'), max_score: $this->input->post('input_skor'), bobot: $this->input->post('input_bobot'));
            $this->jawaban_essay['nilai'] = $final_score['skor_akhir'];
    ...
    private function essay_scoring(string $jawaban, string $kunci_jawaban, int $max_score, int $bobot) {
        // $intersect_array = array_intersect(explode(" ", $kunci_jawaban), explode(" ", $jawaban));
        ...
        $hashing_answer = $this->rolling_hash($tokenized_answer, 5);
        $hashing_key_answer = $this->rolling_hash($tokenized_key_answer, 5);
        $winnowing_answer = $this->winnowing($hashing_answer, 7);
        $winnowing_key_answer = $this->winnowing($hashing_key_answer, 7);
    ...
    private function text_preprocessing(string $kalimat) {
        $case_folding_kalimat = preg_replace('/[^\p{L}\s+]/u', "", strtolower($kalimat));
    ...
    private function cosine_similarity(array $kunci_jawaban, array $jawaban) {
        ...
        if(count($intersection) === 0) {
            ...
            $magnitude1 = sqrt(array_sum(array_map(function($x) { return $x * $x; }, $kunci_jawaban)));
            $magnitude2 = sqrt(array_sum(array_map(function($y) { return $y * $y; }, $jawaban)));
